...
import { getProducts, getFinancialTransactions, getSalesProfitReport, getPurchases, getShipments } from '@/lib/data';
import type { Product, FinancialTransaction, Purchase, Shipment } from '@/lib/types';
...
export default function BalanceSheetPage() {
    ...
    // Manual inputs for items not yet tracked automatically
    ...
    const fetchData = React.useCallback(async () => {
        ...
        try {
            ...
            const [products, financialTransactions, salesReport, allPurchases, allShipments] = await Promise.all([
                getProducts(),
            ...
            const accountsReceivable = allShipments.filter(s => s.paymentStatus === 'Belum Lunas').reduce((acc, s) => acc + s.totalAmount, 0);
            const totalAssets = cash + inventory + accountsReceivable;

            // Calculate Liabilities
            const accountsPayable = allPurchases.filter(p => p.paymentStatus === 'Belum Lunas').reduce((acc, p) => acc + p.totalAmount, 0);
